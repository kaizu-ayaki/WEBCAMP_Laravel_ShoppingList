...
@section('contets')
    ...
    @if (session('Shopping_register_success') == true)
                「買うもの」を登録しました！！<br>
    @endif

    @if (session('list_delete_success') == true)
                「買うもの」を削除しました！！<br>
    @endif

    @if (session('front.shopping_completed_sucsess') == true)
                「買うもの」を完了にしました！！<br>
    @endif
    ...
    @foreach($list as $shopping)
        <tr>
            ...
            <td><form action="{{ route('delete', ['shopping_list_id' => $shopping->id]) }}" method="post">
                    @csrf
                    @method("DELETE")
                    <button onclick='return confirm("この「買うもの」を「削除」します(削除したら戻せません)。よろしいですか？");'>削除</button>
                </form>
    @endforeach
